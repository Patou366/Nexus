...
export default function Tickets() {
  ...
  async function handleSave() {
    ...
    try {
      await saveSection('config', {
        ...
        ticketSupportRoleId: cfg.ticketSupportRoleId,
        ...
        ticketCloseMessage: cfg.ticketCloseMessage,
      })
      setSaved(true)
  ...
  return (
    <div className="max-w-2xl">
      <SectionCard title="Ticket Setup" description="Category and support role for ticket channels.">
        ...
        <Field label="Support Role ID" hint="Role that can view and manage tickets">
          <TextInput value={cfg.ticketSupportRoleId} onChange={v => set('ticketSupportRoleId', v)} placeholder="Role ID…" />
        </Field>
        <Field label="Max Tickets Per User" hint="0 = unlimited">
          <input
            type="number"
            min="0"
            value={cfg.maxTicketsPerUser ?? 1}
      ...
      <SectionCard title="Behaviour" description="Options for how tickets behave when closed.">
        <Field label="DM on Close">
          <Toggle value={cfg.dmOnClose} onChange={v => set('dmOnClose', v)} label="DM the user when their ticket is closed" />
        </Field>
        <Field label="Close Message" hint="Sent in the ticket channel before it's deleted">
          <textarea
            value={cfg.ticketCloseMessage ?? ''}
            onChange={e => set('ticketCloseMessage', e.target.value)}
            placeholder="Your ticket has been closed. Thank you!"
            rows={3}
